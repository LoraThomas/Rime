...
{
    uint8_t    byLen;    /* length of valid command string exclude '\0' */
    uint8_t    a_byBuf[MAX_SIZE_DP_CMD_BUF];    /* buffer of command */	
} DP_CMD;

/* ring buffer to deal with command */
typedef struct _dp_cmd_set
{
    volatile uint8_t    byHead;    /* index of producer command */
    volatile uint8_t    byTail;    /* index of consumer command */
...
************************************************************************************************
*/
int16_t util_GetRoomCBuf(int16_t nHead, int16_t nTail, int16_t nTotNum)
{
    ASSERT( (nTotNum >= 2) && 
...
* Arguments  : uint8_t byChar    received character through USART port
* Returns      : void 
* Notes        : (1) only can called this by DP ISR; 
*                   (2) it would replace carriage-return to '\0';
************************************************************************************************
*/
static void PutCmdChar(uint8_t byChar)
{
    DP_CMD    *p_stCmd;
...
*                    uint8_t byLen    length of desired copy command string 
* Returns      : uint8_t    size of copied command string NOT include '\0' 
* Notes        : (1) only can called this by Monitor-Task; 
*                   (2) Always append '\0';
************************************************************************************************
*/
uint8_t dp_GetCmd(void *p_vSaveBuf, uint8_t byLen)
{
    ASSERT(p_vSaveBuf);
...
*                    int16_t nLen    length of desired TX data 
* Returns      : int8_t    -1=Tx data too long; -2=UART hardware error; 0=TX OK
...
************************************************************************************************
*/
int8_t dp_Tx(const void *p_vSrcBuf, int16_t nSize)
{
